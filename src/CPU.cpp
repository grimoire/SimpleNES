...
void CPU::interrupt(InterruptType type) {
  if (f_I && type != NMI && type != BRK_) return;
...
void CPU::setPageCrossed(Address a, Address b, int inc) {
  ...
  if ((a & 0xff00) != (b & 0xff00)) m_skipCycles += inc;
}
...
void CPU::step() {
  ...
  LOG_CPU << std::hex << std::setfill('0') << std::uppercase << std::setw(4) << +r_PC << "  "
  ...
  if ((opcode & InstructionModeMask) == 0x1) {
    ...
    switch (static_cast<AddrMode1>((opcode & AddrModeMask) >> AddrModeShift)) {
      ...
      case IndirectY: {
        ...
        location = m_bus.read(zero_addr & 0xff) | m_bus.read((zero_addr + 1) & 0xff) << 8;
        if (op != STA) setPageCrossed(location, location + r_Y);
        location += r_Y;
      } break;
      case IndexedX:
      ...
      case AbsoluteX:
        location = readAddress(r_PC);
        r_PC += 2;
        if (op != STA) setPageCrossed(location, location + r_X);
        location += r_X;
        break;
      default:
        return false;
...
Address CPU::readAddress(Address addr) { return m_bus.read(addr) | m_bus.read(addr + 1) << 8; }
