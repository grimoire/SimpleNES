...
PPU::PPU(PictureBus& bus, VirtualScreen& screen)
    : m_bus(bus),
      m_screen(screen),
      m_spriteMemory(64 * 4),
      m_pictureBuffer(ScanlineVisibleDots,
...
void PPU::setInterruptCallback(std::function<void(void)> cb) { m_vblankCallback = cb; }
  ...
  switch (m_pipelineState) {
    ...
    case Render:
      if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
        ...
        if (m_showBackground) {
          ...
          if (!m_hideEdgeBackground || x >= 8) {
            ...
            addr |= m_bgPage << 12;  // set whether the pattern is in the
                                     // high or low page
            // Get the corresponding bit determined by (8 -
            // x_fine) from the right
        ...
        if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
          for (auto i : m_scanlineSprites) {
            ...
            {
              // bit-3 is one if it is the bottom tile of
              // the sprite, multiply by two to get the
              // next pattern
      ...
      if (m_cycle >= ScanlineEndCycle) {
        ...
        // done but (I think) it shouldn't hurt any games if this is done
        // here
    ...
    case PostRender:
      if (m_cycle >= ScanlineEndCycle) {
        ...
        // Should technically be done at first dot of VBlank, but this is
        // close enough
        //                     m_vblank = true;
...
void PPU::control(Byte ctrl) {
  ...
  // Set the nametable in the temp address, this will be reflected in the data address during
  // rendering
