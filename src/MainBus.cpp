...
Byte MainBus::read(Address addr) {
  if (addr < 0x2000)
    ...
    if (addr < 0x4000)  // PPU registers, mirrored
    {
      ...
      if (it != m_readCallbacks.end()) return (it->second)();
...
void MainBus::write(Address addr, Byte value) {
  if (addr < 0x2000)
    ...
    if (addr < 0x4000)  // PPU registers, mirrored
    {
      ...
      if (it != m_writeCallbacks.end()) (it->second)(value);
      // Second object is the pointer to the function object
      // Dereference the function pointer and call it
      else
        LOG(InfoVerbose) << "No write callback registered for I/O register at: " << std::hex
                         << +addr << std::endl;
    ...
    LOG(InfoVerbose) << "Expansion ROM access attempted. This is currently unsupported"
                     << std::endl;
  } else if (addr < 0x8000) {
    if (m_mapper->hasExtendedRAM()) {
