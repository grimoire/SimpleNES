...
Byte PictureBus::read(Address addr) {
  ...
  } else if (addr < 0x3eff)  // Name tables up to 0x3000, then mirrored up to 3eff
  {
...
Byte PictureBus::readPalette(Byte paletteAddr) { return m_palette[paletteAddr]; }
...
void PictureBus::updateMirroring() {
  switch (m_mapper->getNameTableMirroring()) {
    ...
    default:
      NameTable0 = NameTable1 = NameTable2 = NameTable3 = 0;
      LOG(Error) << "Unsupported Name Table mirroring : " << m_mapper->getNameTableMirroring()
                 << std::endl;
  }
}
